#!/usr/bin/env bash
# PROMPT_VARIANT=evidence_first: one ReAct loop per question, no derived layer.
# Two genuinely parallel runs on one 8x A800 (80GB) box.
#
...
# point keeps everything finished so far; re-run extract_evidence.py over the
