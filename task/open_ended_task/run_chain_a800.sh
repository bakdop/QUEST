#!/usr/bin/env bash
# The three-agent question chain, two genuinely parallel runs on one 8x A800 box.
#
# The SLURM cluster caps each user at 2 GPUs, and Qwen3.5-122B-A10B is 234GB so
# it needs TP=4 — one server saturates the quota and a "parallel" pair of jobs
# just serialises. Eight A800s fit two TP=4 servers side by side:
#
#   run A   GPUs 0-3   :8000   -> outputs/<TAG>_a/
#   run B   GPUs 4-7   :8100   -> outputs/<TAG>_b/
#
# Usage (from anywhere, on the A800 box):
#   export SERPER_API_KEY=...  JINA_API_KEY=...
#   bash run_chain_a800.sh
#
#   N=512 WORKERS=16 bash run_chain_a800.sh          # per run
#   RUNS=a           bash run_chain_a800.sh          # only the first half
#   FIXED_PAIRS_FILE=fixtures/pairs_findings8.jsonl N=8 RUNS=a bash run_chain_a800.sh
#
# chain.jsonl is appended one line per completed question, so killing this at any
# point keeps everything finished so far; re-run extract_chain.py over the file
# to pick them up.
set -uo pipefail

# ===================== EDIT THESE IF YOUR PATHS DIFFER =====================
QUEST="${QUEST:-/ssd/boyan/QUEST}"
MODEL="${MODEL:-/ssd/boyan/models/Qwen3.5-122B-A10B}"
VENV_VLLM="${VENV_VLLM:-$QUEST/.venv-vllm}"
VENV_GEN="${VENV_GEN:-$QUEST/.venv}"
# ===========================================================================

N="${N:-512}"                 # questions per run
WORKERS="${WORKERS:-16}"      # in-flight chains per run; each holds 1 live stage
TP="${TP:-4}"                 # 234GB bf16 across 4x80GB, ~86GB left for KV
CTX="${CTX:-65536}"
GPU_UTIL="${GPU_UTIL:-0.92}"
MAX_NUM_SEQS="${MAX_NUM_SEQS:-32}"
SERVED="${SERVED:-qwen3.5-122b}"
RUNS="${RUNS:-a b}"
TAG="${TAG:-chain$(date +%m%d_%H%M)}"
FIXED_PAIRS_FILE="${FIXED_PAIRS_FILE:-}"

SRC="$QUEST/task/open_ended_task"
LOGS="$QUEST/logs"

# This box reaches the internet through a proxy. no_proxy is the load-bearing
# line: vLLM is served on 127.0.0.1, so without it every inference request goes
# out to the proxy and fails. Set PROXY= to run without one.
PROXY="${PROXY-http://10.20.20.18:7890}"
if [[ -n "$PROXY" ]]; then
  export no_proxy="${no_proxy:-127.0.0.1,localhost}" NO_PROXY="${NO_PROXY:-127.0.0.1,localhost}"
  export http_proxy="$PROXY" https_proxy="$PROXY" HTTP_PROXY="$PROXY" HTTPS_PROXY="$PROXY"
fi

say() { echo "[$(date +%H:%M:%S)] $*"; }
die() { echo "FATAL: $*" >&2; exit 1; }

# --------------------------------------------------------------- preflight ---
say "checking prerequisites"
[[ -d "$QUEST" ]]                  || die "QUEST repo not at $QUEST (set QUEST=...)"
[[ -d "$SRC" ]]                    || die "no $SRC — is this the right repo?"
for f in generation_prompt_explore.py generation_prompt_deepen.py \
         generation_prompt_ask.py generate_chain.py extract_chain.py; do
  [[ -f "$SRC/$f" ]] || die "$f missing — pull the branch that has the three-agent chain"
done
[[ -d "$MODEL" ]]                  || die "model not at $MODEL (set MODEL=...)"
[[ -f "$VENV_VLLM/bin/activate" ]] || die "no vllm venv at $VENV_VLLM (set VENV_VLLM=...)"
[[ -f "$VENV_GEN/bin/activate" ]]  || die "no generation venv at $VENV_GEN (set VENV_GEN=...)"
[[ -n "${SERPER_API_KEY:-}" ]]     || die "export SERPER_API_KEY=... (the search tool needs it)"
[[ -n "${JINA_API_KEY:-}" ]]       || die "export JINA_API_KEY=... (the visit tool needs it)"

ngpu=$(nvidia-smi --list-gpus 2>/dev/null | wc -l)
need=$(( TP * $(echo "$RUNS" | wc -w) ))
[[ "$ngpu" -ge "$need" ]] || die "$RUNS needs $need GPUs at TP=$TP but nvidia-smi sees $ngpu"
say "$ngpu GPUs visible, need $need — ok"
mkdir -p "$LOGS"

[[ -n "$PROXY" ]] && say "proxy $PROXY (no_proxy=$no_proxy)"
for host in google.serper.dev r.jina.ai; do
  code=$(curl -s -o /dev/null -w "%{http_code}" --max-time 20 "https://$host/" || echo 000)
  [[ "$code" != "000" ]] || die "no outbound route to $host"
  say "  $host -> $code"
done

# ------------------------------------------------------------------ serving ---
declare -A PORT GPUS PIDS
PORT[a]=8000; GPUS[a]="0,1,2,3"
PORT[b]=8100; GPUS[b]="4,5,6,7"

start_server() {
  local r=$1 p=${PORT[$1]} g=${GPUS[$1]}
  say "run $r: serving $MODEL on GPUs $g, port $p (TP=$TP, ctx=$CTX)"
  (
    source "$VENV_VLLM/bin/activate"
    export CUDA_VISIBLE_DEVICES="$g"
    exec vllm serve "$MODEL" \
      --served-model-name "$SERVED" \
      --host 127.0.0.1 --port "$p" \
      --tensor-parallel-size "$TP" \
      --max-model-len "$CTX" \
      --max-num-seqs "$MAX_NUM_SEQS" \
      --gpu-memory-utilization "$GPU_UTIL" \
      --no-async-scheduling --trust-remote-code
  ) >"$LOGS/${TAG}_${r}_vllm.log" 2>&1 &
  PIDS[$r]=$!
}

cleanup() {
  say "shutting servers down"
  for r in $RUNS; do kill "${PIDS[$r]:-}" 2>/dev/null || true; done
  wait 2>/dev/null || true
}
trap cleanup EXIT INT TERM

for r in $RUNS; do start_server "$r"; done

for r in $RUNS; do
  p=${PORT[$r]}
  say "run $r: waiting for :$p to load (log: $LOGS/${TAG}_${r}_vllm.log)"
  ok=0
  for i in $(seq 1 240); do
    curl -sf "http://127.0.0.1:$p/health" >/dev/null 2>&1 && { ok=1; break; }
    kill -0 "${PIDS[$r]}" 2>/dev/null || die "run $r: vllm died during startup — see $LOGS/${TAG}_${r}_vllm.log"
    sleep 10
  done
  [[ "$ok" -eq 1 ]] || die "run $r: :$p not healthy after 40 min"
  say "run $r: ready"
done

# --------------------------------------------------------------- generation ---
gen_pids=()
for r in $RUNS; do
  p=${PORT[$r]}
  out="$QUEST/outputs/${TAG}_${r}"
  mkdir -p "$out" "$QUEST/database"
  say "run $r: $N questions x 3 stages, $WORKERS workers -> $out"
  (
    cd "$SRC"
    source "$VENV_GEN/bin/activate"
    # The two runs must not share an endpoint, an output dir, or a cache file.
    export LLM_API_BASE="http://127.0.0.1:$p/v1"
    export SERVED_MODEL_NAME="$SERVED"
    export TOKENIZER_PATH="$MODEL" MODEL_PATH="$MODEL"
    export VISIT_CACHE_FILE="$QUEST/database/visit_${r}.db"
    export SEARCH_CACHE_FILE="$QUEST/database/search_${r}.db"
    export FIXED_PAIRS_FILE="$FIXED_PAIRS_FILE"
    # env.local.sh is gitignored, so a fresh clone will not have one. Set what
    # the pipeline actually needs here — including the renamed tool keys, which
    # is the one thing a newcomer cannot guess — and source the file only if it
    # happens to exist, for anything machine-specific beyond this.
    export SERPER_KEY_ID="${SERPER_KEY_ID:-$SERPER_API_KEY}"
    export JINA_API_KEYS="${JINA_API_KEYS:-$JINA_API_KEY}"
    export DEEPRESEARCH_MODEL_NAME="vllm/$SERVED"
    export DEEPRESEARCH_API_BASE="$LLM_API_BASE"
    export DEEPRESEARCH_OPENAI_API_KEY=EMPTY
    export SUMMARY_MODEL_NAME="$SERVED" SUMMARY_OPENAI_API_KEY=EMPTY
    export API_BASE="$LLM_API_BASE" API_KEY=EMPTY
    export SAVE_TRAJ=true
    export VISIT_CACHE_ENABLED=true VISIT_CACHE_RESUME=true
    export SEARCH_CACHE_ENABLED=true SEARCH_CACHE_RESUME=true
    [[ -f "$QUEST/env.local.sh" ]] && source "$QUEST/env.local.sh"
    python generate_chain.py --out_dir "$out" --n "$N" --workers "$WORKERS"
  ) >"$LOGS/${TAG}_${r}_gen.log" 2>&1 &
  gen_pids+=($!)
done

say "both runs going; follow with: tail -f $LOGS/${TAG}_*_gen.log"
say "progress:  wc -l $QUEST/outputs/${TAG}_*/chain.jsonl"
for pid in "${gen_pids[@]}"; do wait "$pid" || say "a generation process exited non-zero; extracting what completed"; done

# ------------------------------------------------------------------ extract ---
for r in $RUNS; do
  out="$QUEST/outputs/${TAG}_${r}"
  n=$( [[ -f "$out/chain.jsonl" ]] && wc -l < "$out/chain.jsonl" || echo 0 )
  say "run $r: $n complete chains"
  [[ "$n" -gt 0 ]] || { say "run $r: nothing to extract"; continue; }
  ( cd "$SRC"; source "$VENV_GEN/bin/activate"
    python extract_chain.py --input "$out/chain.jsonl" \
                            --output "$out/proposed_qa.jsonl" ) | tee "$LOGS/${TAG}_${r}_qc.log"
done

say "DONE"
for r in $RUNS; do
  out="$QUEST/outputs/${TAG}_${r}"
  [[ -f "$out/proposed_qa.jsonl" ]] && echo "  $out/proposed_qa.jsonl  ($(wc -l < "$out/proposed_qa.jsonl") questions)"
  for f in unusable failed; do
    [[ -s "$out/$f.jsonl" ]] && echo "  $out/$f.jsonl  ($(wc -l < "$out/$f.jsonl"))"
  done
done
